...
impl DwarfWarrior {
    pub fn new(name: String, position: HexCoord, terrain: Terrain) -> Self {
        ...
        let magic_resistance = 20 + (base.level as u32 * 2);
    ...
    pub fn break_fortify(&mut self) {
        self.fortified = false;
    }
}

impl Unit for DwarfWarrior {
    fn id(&self) -> UnitId {
        self.base.id
    }
    ...
    fn on_click(&self) {
        println!(
            "Dwarf Warrior {} - Fortified: {}",
            self.name(),
            if self.fortified { "Yes" } else { "No" }
        );
    }
}
