...
impl DwarfMage {
    pub fn new(name: String, position: HexCoord, terrain: Terrain) -> Self {
        ...
        let max_mana = 55 + (base.level as u32 * 11);
    ...
    pub fn add_rune_power(&mut self, amount: u32) {
        self.rune_power = (self.rune_power + amount).min(5);
    }
}

impl Unit for DwarfMage {
    fn id(&self) -> UnitId {
        self.base.id
    }
    ...
    fn move_to(&mut self, position: HexCoord) -> bool {
        if self.can_move_to(position) {
            ...
            // Restore mana (more in mountains)
            let regen = if self.base.current_terrain == Terrain::Mountain {
                4
            } else {
                2
            };
