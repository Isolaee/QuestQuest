...
///
/// `Item` stores metadata and the [`ItemProperties`] which describe the
/// mechanical effects of the item (attack bonuses, defense, consumable
/// effects, etc.). Items are uniquely identified by `ItemId` (a UUID).
#[derive(Clone, Debug, PartialEq, Serialize, Deserialize)]
pub struct Item {
...
impl Item {
    /// Create a new item with autogenerated UUID.
    ///
    /// The `item_type` is inferred from the provided `ItemProperties`.
