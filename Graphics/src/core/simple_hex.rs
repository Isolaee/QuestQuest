...
// Simple, robust hexagonal coordinate system
...
impl SimpleHexCoord {
    ...
    /// This uses the standard formula for flat-top hexagons
    ...
    /// This is the robust inverse transformation
    ...
    /// This ensures we always get the correct hex coordinate
    ...
    pub fn neighbors(self) -> [SimpleHexCoord; 6] {
        [
            SimpleHexCoord::new(self.q + 1, self.r),     // East
            SimpleHexCoord::new(self.q + 1, self.r - 1), // Northeast
            SimpleHexCoord::new(self.q, self.r - 1),     // Northwest
            SimpleHexCoord::new(self.q - 1, self.r),     // West
            SimpleHexCoord::new(self.q - 1, self.r + 1), // Southwest
            SimpleHexCoord::new(self.q, self.r + 1),     // Southeast
        ]
    }
...
/// Simple camera for 2D hexagon world
#[derive(Clone, Debug)]
...
impl SimpleCamera {
    ...
    /// Set camera zoom level (clamped to reasonable values)
    pub fn set_zoom(&mut self, zoom: f32) {
        self.zoom = zoom.clamp(0.1, 10.0);
    }
}

/// Test functions to validate the coordinate system
#[cfg(test)]
mod tests {
    use super::*;
    ...
    fn test_hex_coordinate_conversion() {
        ...
        let test_coords = [
            SimpleHexCoord::new(1, 0),
            SimpleHexCoord::new(0, 1),
            SimpleHexCoord::new(-1, 1),
            SimpleHexCoord::new(-1, 0),
            SimpleHexCoord::new(0, -1),
            SimpleHexCoord::new(1, -1),
        ];
        ...
        for coord in test_coords {
            let pixel = coord.to_pixel(hex_size);
            let back_to_hex = SimpleHexCoord::from_pixel(pixel, hex_size);
            assert_eq!(coord, back_to_hex, "Failed for coordinate {:?}", coord);
        }
    }
